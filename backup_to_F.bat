...
echo(
"%SystemRoot%\System32\robocopy.exe" "%SRC%" "%DST%" /MIR /XJ /SL /XD __pycache__ .venv venv venv.incompatible.* env .tmp node_modules runtime-data diagnostics shared_logs quarantine flight-recorder baselines remediations heartbeats ipc models secrets drill-sandbox staged_patches shadow_cache forensics build dist .dev-tools .pytest_cache .mypy_cache .ruff_cache .hypothesis htmlcov .tox pip-wheel-metadata .eggs "%SRC%\logs" "%SRC%\analysis\manual_build" LibreOffice_* /XF *.pyc .env .env.* settings.json *.settings.json user_config.json credentials.json credentials.yaml credentials.yml secrets.json secrets.yaml secrets.yml tokens.json api_keys.json auth.json client_secret*.json client_secret*.yaml client_secret*.yml client_secret*.txt id_rsa id_ed25519 .npmrc .pypirc .netrc *.key *.pem *.pfx *.p12 *.token *.secret *.secrets ANGERONA_WATCHDOG_TOKEN* bus.key *.db *.sqlite *.sqlite3 *.log *.gguf *.hb *.ring *.bak *.tmp *.lnk *.test *.spec .coverage custom_user_patch.ps1 standdown.cmd selfcheck_report*.txt angerona_watchdog.exe frz_watchdog.exe /R:1 /W:1 /NP /NFL /NDL /NJH /NJS /NC /NS
set "RC=%ERRORLEVEL%"
if %RC% GEQ 8 (
...
echo [DONE] Backup complete. Robocopy status %RC% is successful.
goto finish
