...
<sub>*Fresh 1920×1040 synthetic public-demo capture. It is generated by `tools/capture_public_dashboard.py` from fixed synthetic values (66 modules, posture 96 Secure, and 0 alerts); no host telemetry, identity, path, PID, or network address is displayed.*</sub>
...
- **Network-first Chill mode** — the default unattended posture keeps network,
  endpoint-event, USB, response, and resilience sentinels active while parking
  deep disk/AI work and spacing maintenance cycles. Ollama is unloaded while
  idle; an authenticated active-threat event wakes the full defense path.
- **Active-threat-only global posture** — drills, vulnerability exposure,
  detector health, Defender state, and ordinary USB insertion remain visible in
  their own surfaces without declaring the host Critical. High/Critical global
  posture is reserved for evidence classified as an active threat or attack.
- **Bounded local response and device trust** — Live Alerts can queue a response
  directly; SOAR exposes review/approve/dismiss/execute controls; Scan Center
...
If the launcher finds an old or unsupported virtual environment, run
`Repair-Angerona-Python.bat`. The repair requires an explicit typed confirmation,
preserves the previous environment for rollback, obtains only an
Authenticode-verified CPython 3.12 x64 interpreter through the trusted Windows
package path when needed, and rebuilds from the SHA-256-locked wheel set. The
launcher will not bypass this ABI gate with an existing CPython 3.14 environment.
...
## LATEST (2026-08-22 — Cycle 22 unattended defense and verified remediation)

- **All-day, network-first Chill mode.** Windows autostart enters Chill mode.
  Network, ETW/Sysmon, WFP, AV, USB, SOAR, and resilience sentinels remain
  available while deep file/memory/YARA/deception work is parked or placed on a
  sparse sequential maintenance cadence. The local model is unloaded when idle;
  an authenticated active-threat revision wakes the fuller defense path. This
  reduces idle disk activity and avoids keeping Ollama/llama3 resident solely
  for background monitoring. Live profiling also moved presentation to a 5 s
  active-Chill, 10 s inactive, and 15 s hidden cadence with elapsed-time-
  equivalent panel periods; High/Critical evidence coalesces an immediate GUI
  wake. ARIA animation stops in Chill/inactive/hidden states, and minimized
  watchdog/scanner UI refreshes every 10 s with unchanged-list caching. Sensor
  and supervisor recovery cadence is unchanged.
- **Active-threat-only posture.** Event severity is now distinct from event
  disposition. Exact registered drill evidence, vulnerability exposure,
  detector health, Defender state, and USB insertion remain reviewable without
  forcing the host to Critical. Only an active-threat/active-attack disposition
  can drive global High or Critical posture; poor configuration remains
  Elevated.
- **Response, scan, and removable-media controls.** Live Alerts can queue a
  bounded action without forcing a tab switch; SOAR exposes approve, dismiss,
  and execute controls over authenticated records. Scan Center uses responsive
  layouts, cancellable workers, and honest Defender activity/liveness rather
  than a fabricated percentage. USB approval requires a protected PIN and
  binds trust to volume identity; wrong attempts lock, reset revokes prior
  trust, and a changed device behind the same mount point must be approved again.
- **D:-resident source state and privacy-safe deception.** The Windows source
  launcher overwrites inherited storage settings with its sibling
  `AngeronaData` directory on D:, and product reports, databases, scanner
  evidence, diagnostics, temporary work, and sidecar state use canonical data
  helpers. Deception now defaults inside that same data root; Desktop,
  Documents, Downloads, and other personal folders require an explicit Settings
  or Full Setup opt-in. Migration rejects links, junctions, special files, and
  overlapping source/destination roots.
- **Crash and recovery hardening.** Asynchronous Qt dialogs/panels reject late
  callbacks during close and wait for workers before deletion. The resilience
  path uses fixed-size HMAC-authenticated, replay-checked per-component
  heartbeats; invalid, legacy, or replayed records cannot authorize process
  control. FRZ now emits the same authenticated 32-byte heartbeat-v2 record
  instead of its legacy 16-byte raw map. Spawn-lock I/O uncertainty fails closed
  and retries within existing restart budgets.
- **Credential and native-build boundaries.** JARVIS remote-control authority
  is accepted only from Angerona's protected store; inherited token variables
  are scrubbed before elevation and runtime, while Setup provides masked
  enrollment and regeneration. FRZ v2 builds/searches only as
  `frz/frz_watchdog_v2.exe`, and its Go module dependencies are pinned. Go was
  unavailable on the verification host, so the native FRZ binary build remains
  an external gate.
- **Proof-bound red-team closure.** Apply Fix Candidates installs an exact
  report-bound detector candidate. Test Fix executes distinct positive and
  negative controls and requires authenticated EventBus detection, persisted
  detector evidence, a real Active Response SOAR receipt, cleanup, and a signed
  verification receipt. A source drill cannot certify itself; only a distinct
  later run with the exact action contract can reach verified closure, and an
  expiry or later miss reopens the gap. This proves a fixed gap; it does not
  promise that every drill automatically reaches 100%.
- **Privacy-preserving case correlation.** Cases can hold bounded typed
  observables with suggested/approved/rejected review states. Similar-case
  matching uses a derived-key HMAC private index and only approved, non-excluded
  records; tampered rows are refused. Sanitized exports contain aggregate counts,
  not raw or normalized observables, sources, or correlation values. The design
  follows the human-review patterns in [TheHive observables](https://docs.strangebee.com/thehive/user-guides/analyst-corner/cases/observables/about-observables/)
  and [similar cases](https://docs.strangebee.com/thehive/user-guides/analyst-corner/cases/find-similar-alerts-cases/)
  without claiming distributed enterprise parity.
- **Launcher and publication safety.** Hardened Windows source launch now
  enforces the reviewed CPython 3.12 x64 ABI even when an incompatible virtual
  environment already exists. `Repair-Angerona-Python.bat` provides an explicit,
  rollback-preserving repair from verified Python plus hash-locked wheels. The
  bootstrap pins reviewed pip 26.2.1 by its verified SHA-256 requirement before
  dependency installation and refuses an unexpected pip version. The
  protected-task validator recognizes Windows' omitted `RunLevel` as the
  serialized Limited default only for the expected interactive principal. The
  Git push helper aborts after add/commit failure and passes the commit message
  through a UTF-8 file instead of shell interpolation. Manual release branch
  names map to deterministic path-safe artifacts without changing tag names.
  The public dashboard image was regenerated from fixed synthetic-only data.
- **Fail-closed F: backup boundary.** `backup_to_F.bat` validates the repository
  root and fixed F: destination, rejects reparse/junction and path-escape cases,
  excludes secrets plus runtime/cache/model/build state, and removes stale
...
- **Remaining boundary.** Physical sleep/resume and long elevated-host soaks,
  ...
  multi-node HA, mTLS/OIDC, and independent penetration/DR evidence remain
  external gates. Angerona is a broad local security workbench, not a claim of
  replacement-level parity with every distributed enterprise platform.
- **Current-host startup repair.** The Python repair completed and the protected
  autostart entry was live-reconciled to CPython 3.12
