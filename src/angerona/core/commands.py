    ...
    def _ingest_status(self, args: List[str]) -> str:
        ...
        if self.flight_recorder_worker is None:
            ...
            recorder_text = (
                ...
                f"  overflow queue: {durable.overflow_queue_depth}/"
                f"{durable.overflow_queue_capacity}\n"
                ...
                f"  replayed/duplicates/quarantined/failures: "
                f"{durable.replayed}/{durable.replay_duplicates}/"
                f"{durable.replay_quarantined}/{durable.replay_failures}\n"
                f"  spool: {durable.spool_segments}/"
                f"{durable.spool_max_segments} segments, "
                f"{durable.spool_bytes}/{durable.spool_max_bytes} bytes, "
                f"quarantine={durable.spool_quarantine_segments} segment(s)/"
                f"{durable.spool_quarantine_bytes} bytes, "
                f"capacity-waits={durable.spool_capacity_waits}/"
                f"{durable.spool_capacity_wait_seconds:.2f}s\n"
