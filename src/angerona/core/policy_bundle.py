...
class PolicyBundle:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.layer, PolicyLayer) or not isinstance(
            self.rollout, RolloutState
        ):
            raise ValueError("layer and rollout must use their typed enums")
        if not self.bundle_id or self.version < 1 or not self.publisher_id:
            raise ValueError("invalid bundle identity/version")
        if self.channel not in {"configuration", "detection", "remediation"}:
        ...
        if len(keys) != len(set(keys)) or any(not key or len(key) > 200 for key in keys):
            raise ValueError("setting keys must be unique and bounded")
        if not set(self.locked_keys) <= set(keys):
    ...
    *,
    ...
    try:
        Ed25519PublicKey.from_public_bytes(publisher).verify(
            _unb64(bundle.signature), bundle.canonical_signed()
        )
    except Exception:
        ...
        for approval in bundle.approvals:
            ...
            try:
                Ed25519PublicKey.from_public_bytes(public).verify(
                    _unb64(approval.signature), body
                )
...
def resolve_effective(
    ...
    for bundle in selected:
        ...
        if current is None or (bundle.version, bundle.bundle_id) > (
            current.version, current.bundle_id
        ):
        ...
        for key, value in sorted(bundle.settings):
            if key in locked:
                diffs.append(PolicyDiff(
                    key, values.get(key), value, bundle.bundle_id, True
                ))
                continue
    ...
    return EffectivePolicy(
        channel, tuple(sorted(values.items())), tuple(sorted(sources.items())),
        tuple(sorted(locked)), tuple(used),
    ), tuple(diffs)
...
class PolicyManager:
    ...
    def __init__(
        self, trusted_publishers: Mapping[str, bytes],
    ...
    def submit(self, bundle: PolicyBundle, *, now: float | None = None) -> tuple[bool, str]:
        valid, reason = verify_bundle(
            bundle, self.publishers, self.approvers, now=now
        )
        if not valid:
            return False, reason
    ...
    def effective(
        ...
        stamp = time.time() if now is None else float(now)
        active = [
            bundle for bundle in self._good.values()
    ...
    def receipt(
        ...
        *, now: float | None = None,
        ...
        core = {
            ...
            "publisher_id": bundle.publisher_id, "channel": bundle.channel,
            "recorded_at": stamp,
        ...
        return PolicyAuditReceipt(
            **core, receipt_hash=hashlib.sha256(_canonical(core)).hexdigest()
        )
