...
class ResponseOperation:
    ...
    def __post_init__(self) -> None:
        if not _OP.fullmatch(self.operation_id):
            raise ValueError("invalid operation ID")
        if self.risk not in _RISKS:
            raise ValueError("invalid response risk")
        if not self.description.strip() or not self.rollback.strip():
...
class ResponseBroker:
    ...
    def execute(self, proposal: ResponseProposal) -> ResponseReceipt:
        with self._lock:
            ...
            if self._clock() >= proposal.expires_at:
                raise PermissionError("response proposal expired")
            operation.validator(proposal.arguments)
            ...
            if not proposal.dry_run and len(approvals) < required:
                raise PermissionError(
                    f"{operation.risk} response requires {required} approval(s)"
                )
            if proposal.dry_run:
                result: Mapping[str, Any] = {
                    "validated": True, "risk": operation.risk,
    ...
    def verify_receipt(self, receipt: ResponseReceipt) -> bool:
        ...
        return hmac.compare_digest(
            signature, hmac.new(
                self._key, _canonical(value), hashlib.sha256
            ).hexdigest(),
        )
