...
class EvidenceLattice:
    ...
    def ingest(self, event: Event, now: float | None = None) -> EvidenceFinding | None:
        ...
        if event.module == EvidenceLatticeModule.name or event.severity != Severity.MEDIUM:
            return None
        # Never promote calibrated observations, practice, passive exposure,
        # or suite health into a fabricated HIGH incident merely because they
        ...
        try:
            ...
            if event_disposition(event) in {"observation", "practice", "exposure", "health"}:
                return None
        except Exception:
