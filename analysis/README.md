...
| Linux | **Observe + optional eBPF** | Rootless process/flow/posture observation; privileged BCC/eBPF is an explicit supplement |
...
<sub>*Historical 1920×1040 synthetic public-demo capture. It was generated by `tools/capture_public_dashboard.py` from fixed synthetic events and resource values; no host telemetry, identity, path, PID, or network address is displayed. That archived capture represented 71 modules; current discovery is documented in the root README.*</sub>
...
## Historical v1.10.3 — Cycle 23 state-grade-pattern defenses

Cycle 23 added three actor-neutral, observe-only guards: SSH Surface / Key /
Tunnel Guard, Windows Audit Log Integrity Guard, and Zero-Trust Network Path
Monitor. They translate public advanced-operator reporting into bounded evidence
for SSH configuration/key/runtime drift, explicit log clears and continuity
loss, and restart-safe Wi-Fi/Ethernet route/DNS/DHCP/gateway/profile changes.
The signals describe techniques; they do not establish agency or state
attribution.

The optional Personal Sentinel component is a pinned, direct-HTTPS
**attestation client** for this exact topology:

`Angerona host -> operator-controlled Personal Sentinel gateway/firewall -> upstream/ISP router -> Internet`

Normal TLS/hostname verification, a certificate pin, nonce/freshness, an
expected policy digest, optional mutual TLS, complete IPv4/IPv6 route evidence,
and pre/post route-context equality can attest only one enrolled path. No
gateway appliance/server or routing role, router discovery/management,
credential storage, endpoint trust, or firmware attestation is shipped.

The dashboard's Live Defense Activity card shows no more than five sanitized
public EventBus/module summaries. It is not executing source, raw telemetry,
hidden model reasoning, or chain-of-thought. ARIA now receives a bounded,
strict-schema, canonical-SHA-256-pinned, data-only Defense Memory through local
runbook retrieval for quick capability, usage, defensive-control, and
actor-neutral tradecraft questions.

Audit/network state has an injected independent-high-water protocol, but no
separately administered monotonic service or policy-bound hardware authority is
bundled. Default state is `local-authenticity-only` with
`independent_freshness_verified=false`; the Personal Sentinel compact receipt
and in-memory tests are not independent anti-rollback custody.

Final Windows evidence: **1,460 passed / 5 expected host-capability skips / 0
failed** from 1,465 tests across 208 files; product compile **321/321**; module
files **73/73** imported; modules **71/71** discovered; compatibility hooks
**58/58**; standalone core/Shark self-tests **22/22**; module harness **50 pass /
0 fail / 21 expected skips** plus EventBus PASS; direct and batch selfcheck
**26/26**; Ruff clean. Full three-round disposition, performance evidence,
primary-source URLs, proposals, and limits are in
[`loop/cycle23/summary.md`](loop/cycle23/summary.md).
...
<!-- ANGERONA_ARCHIVE_STATUS tests=1460 skips=5 modules=71 -->
