...
## Current development — Cycle 5

- Every primary dashboard card, panel, ARIA/console surface, System Pulse tile,
  and bottom module-status chip opens a bounded live detail view through the
  reduced-motion-aware real-window reveal.
- The Red Team console is resizable and screen-aware. Its configuration scrolls
  on smaller displays while Launch and Stop remain in a sticky reachable footer.
...
## Current development — Cycle 6

- Teams development authentication is ephemeral and direct-loopback-only;
  forwarded, tunnelled, non-local, and persisted-setting paths require JWT.
- EventBus and shutdown commands use distinct protected authorities. Elevated
  startup quarantines unsafe pre-existing keys before use. This user-mode ACL
  boundary does not defeat a compromised Administrator/SYSTEM principal.
- The optimized GUI telemetry cursor verifies persisted Event HMACs and rejects
  invalid rows. A bounded, authenticated, read-only kernel-boundary posture
  ledger reports Code Integrity/HVCI/VBS, vulnerable-driver controls, driver
  state, and source health without claiming kernel enforcement.
- WFP containment now has typed expiry-bound plans, recovery exclusions,
  exact-plan approval, rollback receipts, and apply-time reconstruction. It
  remains preview-first and needs a separate privileged broker for enforcement;
  there is no silent firewall mutation.
- Live coverage accounting exposes gaps, duplicates, regressions, missing
  metadata, staleness, and bounded sensor eviction. Resolve Center renders 25
  rows per page and unchanged refreshes avoid table reconstruction.
- Recorded gates: **223 passed / 2 intentional platform skips**, compile
  **233/233**, discovery **65 modules / 0 errors**, self-check **26/26**.
- The editable source checkout remains weaker than the signed,
  Administrator-owned package. Angerona remains user-mode, not tamper-proof or
  enterprise-certified; the driver remains lab-only.
